...
public class RawDbPlayer {
    ...
    public RawDbPlayer copy() {
        return new RawDbPlayer(id, color, ps, aiLevel, isWinner, elo, isOfferingDraw, lastDrawOffer, isProposingTakeback, userId, eloDiff, mts, blurs);
    }
    ...
    public Optional<DbPlayer> decode() {
        return Color.apply(color).map(trueColor -> new DbPlayer(
            ...
            Optional.ofNullable(mts).orElse(""),
    ...
    public static RawDbPlayer encode(DbPlayer dbPlayer) {
        return new RawDbPlayer(
            ...
            dbPlayer.getIsProposingTakeback(),
            dbPlayer.getUserId().orElse(null),
            null,
            dbPlayer.getMoveTimes().isEmpty() ? null : dbPlayer.getMoveTimes(),
